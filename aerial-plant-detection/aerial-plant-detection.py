...
image2=image
image3=np.ones((3000,3000,3),np.uint8)
x1,y1,x2,y2=0,0,300,300
for y in range(10):
    x1,x2=0,300
    for x in range(10):
        img1=image[y1:y2,x1:x2]
        image_expanded = np.expand_dims(img1, axis=0)
    ...
    # Draw the results of the detection (aka 'visulaize the results')
        ...
        for i in range(100):
            if scores is None or final_score[i] > 0.6:
                count = count + 1
        im_height, im_width = img1.shape[:2]
        ...
        for i in range(0,1,1):
            ...
            img2=cv2.line(img2,(a,ymin),(a,ymax),(0,255,0),5)
        #print(xmin, xmax, ymin, ymax)
        '''

        #print(count)
        image2[y1:y2,x1:x2]=img1
        #image3[y1:y2,x1:x2]=img2
        # All the results have been drawn on image. Now display the image.
        #cv2.imshow(str(x)+str(y)+'Object detector', img1)
        #cv2.waitKey(0)
        data[y][x]=count
        # Clean up
